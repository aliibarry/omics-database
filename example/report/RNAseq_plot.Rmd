...
deg_df_list = params$deg_df_list 
...
if (('mgi_symbol' %in% colnames(count_df)) == FALSE) {
...
tcounts_med %>% as.data.frame %>% head
```
...
if (include_deg == TRUE) {
  ...
  deg = final_df %>% filter(symbol %in% geneids)
  deg %>% as.data.frame %>% head
  # plotting 
...
for (volc_pop in pop_list) {
  res = as.data.frame(pop_df_dic[[volc_pop]])
